...
///
/// Deliberately smaller than A2A's task lifecycle
/// (`submitted`/`working`/`input-required`/`completed`/`failed`/
/// `canceled`/`rejected`) since this package has no notion of
/// multi-party delegation or mid-task input requests. If you bridge a
/// [Result] to an A2A `Task`, [maxStepsReached] has no A2A equivalent
/// and should map to `failed`.
enum AgentStatus {
