...
class Result extends Equatable {
  ...
  /// tool call/result pairs — in chronological order.
  ...
  ///
  /// `null` for agents (e.g. process-based CLI agents) that don't have
  ...
  factory Result.fromJson(Map<String, dynamic> json) => Result(
        ...
        usage: json['usage'] != null
            ? Usage(
                inputTokens: (json['usage']
                        as Map<String, dynamic>)['inputTokens'] as int? ??
                    0,
                outputTokens: (json['usage']
                        as Map<String, dynamic>)['outputTokens'] as int? ??
                    0,
                totalTokens: (json['usage']
                        as Map<String, dynamic>)['totalTokens'] as int? ??
                    0,
              )
            : null,
  ...
  ///
  /// 1. The last assistant message that contains text — the natural
  ///    output for CLI-style agents, which always produce a final text
  ///    summary.
  /// 2. The last tool-result message that contains text — handles agents
  ///    whose loop terminates after a tool call without a follow-up
  ///    text turn (e.g. the final step is `bash echo "done"`).
  ///
  /// Returns `null` only if neither source produced any text.
  String? get outputText {
    // 1. Prefer the last assistant text response.
